...
from hoid_model.utils.viz_hand_obj import *
    ...
    for i in range(np.minimum(10, dets.shape[0])):
        ...
        if score > thresh:
            ...
            if class_name == "hand":
                cv2.putText(
                    im,
                    "%s: %.3f lr %.1f s %.1f" % (class_name, score, lr, state),
                    (bbox[0], bbox[1] + 15),
    ...
    if (obj_dets is not None) and (hand_dets is not None):
        ...
        for i in range(np.minimum(10, hand_dets.shape[0])):
            ...
            if score > thresh:
                ...
                cv2.putText(
                    im,
                    "%s: %.3f lr %.1f s %.1f" % ("hand", score, lr, state),
                    (bbox[0], bbox[1] + 15),
                    cv2.FONT_HERSHEY_PLAIN,
                    1.0,
                    (0, 0, 255),
...
def vis_detections_filtered_objects_PIL(
    ...
    thresh_hand=0.8,
    thresh_obj=0.01,
    font_path="lib/hoid_model/utils/times_b.ttf",
):
...
def vis_detections_PIL(
    im, class_name, dets, thresh=0.8, font_path="lib/hoid_model/utils/times_b.ttf"
):
    """Visual debugging of detections."""
...
def filter_object(obj_dets, hand_dets):
    """
    find one of the closest object for each hand
    """
    filtered_object = []
    object_cc_list = []
