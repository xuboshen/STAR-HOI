...
def ratio2coord(ratio, width, height):
    ...
    x1, y1, x2, y2 = (
        int(float(ratio[0]) * width),
        int(float(ratio[1]) * height),
        int(float(ratio[2]) * width),
        int(float(ratio[3]) * height),
    )
...
def draw_obj_mask(image, draw, obj_idx, obj_bbox, obj_score, width, height, font):
    ...
    draw.text(
        (obj_bbox[0] + 5, max(0, obj_bbox[1] - 30) - 2), f"O", font=font, fill=(0, 0, 0)
    )  #
...
def draw_hand_mask(
    image, draw, hand_idx, hand_bbox, hand_score, side, state, width, height, font
):
    ...
    pmask = ImageDraw.Draw(mask)
    pmask.rectangle(
        hand_bbox, outline=hand_rgb[side_idx], width=4, fill=hand_rgba[side_idx]
    )
    image.paste(mask, (0, 0), mask)
    # text
    ...
    draw.rectangle(
        [
            hand_bbox[0],
            max(0, hand_bbox[1] - 30),
            hand_bbox[0] + 62,
            max(0, hand_bbox[1] - 30) + 30,
        ],
        fill=(255, 255, 255),
        outline=hand_rgb[side_idx],
        width=4,
    )
    draw.text(
        (hand_bbox[0] + 6, max(0, hand_bbox[1] - 30) - 2),
        f"{side_map3[int(float(side))]}-{state_map2[int(float(state))]}",
        font=font,
        fill=(0, 0, 0),
    )  #
