...
class extension_layer(nn.Module):
    ...
    def forward(self, input, input_padded, roi_labels, box_info):
        ...
        loss_list = [
            self.hand_contactstate_part(input_padded, roi_labels, box_info),
    ...
    def init_layers_weights(self):
        ...
        self.hand_contact_state_layer = nn.Sequential(
            nn.Linear(2048, 32), nn.ReLU(), nn.Dropout(p=0.5), nn.Linear(32, 5)
        )
    ...
    def hand_contactstate_part(self, input, roi_labels, box_info):
        ...
        if self.training:
            for i in range(input.size(0)):
                ...
                if index.sum() > 0:
                    contactstate_loss_sub = 0.1 * self.hand_contactstate_loss(
                        contactstate_pred[i][index], gt_labels[index].long()
                    )
    ...
    def hand_dxdymagnitude_part(self, input, roi_labels, box_info):
        ...
        dxdymagnitude_pred_sub = 0.1 * F.normalize(
            dxdymagnitude_pred[:, :, 1:], p=2, dim=2
        )

        dxdymagnitude_pred_norm = torch.cat(
            [dxdymagnitude_pred[:, :, 0].unsqueeze(-1), dxdymagnitude_pred_sub], dim=2
        )
        ...
        if self.training:
            ...
            for i in range(input.size(0)):
                ...
                if index.sum() > 0:
                    dxdymagnitude_loss_sub = 0.1 * self.hand_dxdymagnitude_loss(
                        dxdymagnitude_pred_norm[i][index], gt_labels[index]
                    )
    ...
    def hand_handside_part(self, input, roi_labels, box_info):
        ...
        if self.training:
            for i in range(input.size(0)):
                ...
                index = roi_labels[i] == 2  # if class is hand
                if index.sum() > 0:
                    handside_loss_sub = 0.1 * self.hand_handside_loss(
                        handside_pred[i][index], gt_labels[index].unsqueeze(-1)
                    )
