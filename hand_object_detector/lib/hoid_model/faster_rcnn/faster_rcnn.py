...
from hoid_model.extension_layers import extension_layers
from hoid_model.roi_layers import ROIAlign, ROIPool
from hoid_model.rpn.proposal_target_layer_cascade import _ProposalTargetLayer
from hoid_model.rpn.rpn import _RPN
from hoid_model.utils.config import cfg
from hoid_model.utils.net_utils import (
    _affine_grid_gen,
    _affine_theta,
    _crop_pool_layer,
    _smooth_l1_loss,
...
# from hoid_model.roi_pooling.modules.roi_pool import _RoIPooling
# from hoid_model.roi_align.modules.roi_align import RoIAlignAvg
...
class _fasterRCNN(nn.Module):
    ...
    def forward(self, im_data, im_info, gt_boxes, num_boxes, box_info):
        ...
        # feed image data to base hoid_model to obtain base feature map
        ...
        # feed pooled features to top hoid_model
        ...
        if self.training:
            ...
            # auxiliary layer
            loss_list = self.extension_layer(
                pooled_feat, pooled_feat_padded, rois_label_retain, box_info
            )
        else:
            loss_list = self.extension_layer(
                pooled_feat, pooled_feat_padded, None, box_info
            )
    ...
    def enlarge_bbox(self, im_info, rois, ratio=0.5):
        rois_width, rois_height = (rois[:, :, 3] - rois[:, :, 1]), (
            rois[:, :, 4] - rois[:, :, 2]
        )
