...
class DenseAffine3DGridGen(Module):
    ...
    def forward(self, input1):
        self.batchgrid3d = torch.zeros(
            torch.Size([input1.size(0)]) + self.grid3d.size()
        )
...
class Depth3DGridGen(Module):
    ...
    def forward(self, depth, trans0, trans1, rotate):
        ...
        x = self.batchgrid3d[:, :, :, 0:1] * depth + trans0.view(-1, 1, 1, 1).repeat(
            1, self.height, self.width, 1
        )

        y = self.batchgrid3d[:, :, :, 1:2] * depth + trans1.view(-1, 1, 1, 1).repeat(
            1, self.height, self.width, 1
        )
...
class Depth3DGridGen_with_mask(Module):
    ...
    def forward(self, depth, trans0, trans1, rotate):
        ...
        x_ = self.batchgrid3d[:, :, :, 0:1] * depth + trans0.view(-1, 1, 1, 1).repeat(
            1, self.height, self.width, 1
        )

        y_ = self.batchgrid3d[:, :, :, 1:2] * depth + trans1.view(-1, 1, 1, 1).repeat(
            1, self.height, self.width, 1
        )
        z = self.batchgrid3d[:, :, :, 2:3] * depth
        # print(x.size(), y.size(), z.size())
        ...
        if depth.is_cuda:
            phi = torch.atan(y / (x + 1e-5)) + np.pi * x.lt(0).type(
                ...
                y.ge(0).type(torch.cuda.FloatTensor)
