...
class pascal_voc(imdb):
    def __init__(self, image_set, year, devkit_path=None):
        ...
        self._classes = ("__background__", "targetobject", "hand")  # always index 0
    ...
    def _get_default_path(self):
        ...
        default_path = os.path.join(
            cfg.DATA_DIR, "VOCdevkit" + self._year + "_handobj_100K"
        )
        print()
    ...
    def _load_pascal_annotation(self, index):
        ...
        for ix, obj in enumerate(objs):
            ...
            mag = (
                0 if self._is_not_legimate(mag) else float(mag.text) * 0.001
            )  # balance scale
            magnitude[ix] = mag
            ...
            with open(filename, "wt") as f:
                for im_ind, index in enumerate(self.image_index):
                    ...
                    for k in xrange(dets.shape[0]):
                        f.write(
                            "{:s} {:.3f} {:.1f} {:.1f} {:.1f} {:.1f} {:.1f} {:.3f} {:.3f} {:.3f} {:.3f} {:.3f}\n".format(
                                index,
                                dets[k, 4],
                                dets[k, 0] + 1,
                                dets[k, 1] + 1,
        ...
        for i, cls in enumerate(self._classes):
            ...
            if cls == "hand":
                ...
                for constraint in ["handstate", "handside", "objectbbox", "all"]:
                    ...
                    with open(
                        os.path.join(output_dir, cls + f"_pr_{constraint}.pkl"), "wb"
                    ) as f:
                        pickle.dump({"rec": rec, "prec": prec, "ap": ap}, f)
    ...
    def _do_matlab_eval(self, output_dir="output"):
        ...
        print("-----------------------------------------------------")
        path = os.path.join(
            cfg.ROOT_DIR, "lib", "hoid_datasets", "VOCdevkit-matlab-wrapper"
        )
        cmd = "cd {} && ".format(path)
