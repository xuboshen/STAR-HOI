...
def parse_args():
    ...
    parser.add_argument(
        "--save_name", help="folder to save eval results", required=True
    )
    parser.add_argument("--thresh_hand", type=float, default=0.1, required=False)
    parser.add_argument("--thresh_obj", default=0.1, type=float, required=False)
...
if __name__ == "__main__":
    ...
    input_dir = (
        args.load_dir + "/" + args.net + "_" + args.model_name + "/" + args.dataset
    )
    ...
    for i in range(num_images):
        ...
        nc_prob = F.softmax(
            hand_contacts[:, :, 0].squeeze(0).unsqueeze(-1).float().detach()
        )
